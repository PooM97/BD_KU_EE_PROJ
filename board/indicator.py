...
class IndicatorManager():
    ...
    def __init__(self, config) -> None:        
        ...
        typecom_list =  [
            ...
            [serial.PARITY_NONE, 2],
            [serial.PARITY_ODD, 1],
            [serial.PARITY_ODD, 2],
            [serial.PARITY_EVEN, 1],
            [serial.PARITY_EVEN, 2]
        ]
        commu = config['communication']
    ...
    def __create_boxes(self):
        ...
        for box in self.config['boxes']:            
            ...
            box.serial.timeout  = 0.1 # Read timeout value in seconds.s
            box.close_port_after_each_call = True
    
    def writeIndicator(self, box_name, register, value, decimal=0):
        try:
            box: minimalmodbus.Instrument = self.boxes[box_name]
            box.write_register(register, value, decimal, self.functioncode)
        except IOError: 
